...
let draftSeedAutoGenerated = false;
...
function currentDraftSeed(refreshAutoSeed = false) {
  let seed = els.draftSeed.value.trim();
  if (!seed || (refreshAutoSeed && draftSeedAutoGenerated)) {
    seed = createRandomDraftSeed();
    els.draftSeed.value = seed;
    draftSeedAutoGenerated = true;
  }
  return seed;
}
...
function draftPlayerForTeam(teamIndex, player, forced = false) {
  ...
  const pickIndex = draftState.pickIndex;
  const round = Math.floor(pickIndex / draftState.teamCount) + 1;
  const pick = (pickIndex % draftState.teamCount) + 1;
  const targetRole = slotRoleForTeamSlot(teamSlot).label;
  const record = {
    pickIndex,
    round,
    pick,
...
function renderDraftResults() {
  ...
  if (!draftState?.picks?.length) {
    ...
    els.draftResults.textContent = supportedDraftLoaded()
      ? "Start a draft to see each team's picks by round."
      : "Load a supported 28-team NES football ROM to show draft results.";
    return;
  }

  els.draftResults.className = "draft-results";
  const rows = draftState.picks
    .slice()
    .sort((a, b) => a.pickIndex - b.pickIndex)
    .map((pick) => `
      <article class="draft-result-card">
        <div class="draft-result-pick">R${pick.round} P${pick.pick}</div>
        ${facePreviewHtml(pick.faceId, "draft-face")}
  ...
  els.draftResults.innerHTML = `
    <div class="draft-result-heading">
      <strong>${draftState.picks.length} pick${draftState.picks.length === 1 ? "" : "s"}</strong>
      <span>Seed ${escapeHtml(draftState.seed)}</span>
    </div>
    <div class="draft-result-list">${rows}</div>
  `;
}
...
els.draftSeed.addEventListener("input", () => {
  draftSeedAutoGenerated = false;
});
